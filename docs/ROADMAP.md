...
| **7** | **異常値検知**（閾値・先週比）とレポート内アラート／総合スコア自動計算 | ✅ 実装済 | `src/generate_alerts.py`。売上・MER・全体ROAS・広告CPA/CPC/CVR/ROASの悪化を検知し、`report.alert` / `report.alerts` を HTML に反映。アラート件数・重要度と主要KPIから `overall_score` / `score_subtitle` / `score_meta` も更新 |
