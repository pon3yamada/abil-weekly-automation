...
- **症状**: 7/11 にトークンを再発行したのに「失効済み」Slack通知が毎日届き続けた。
- **原因**: `token_expiry_check` は Secrets の `META_TOKEN_GENERATED_AT` + 60日で残日数を計算しており、トークン再発行時に `META_ACCESS_TOKEN` だけ更新されて日付が旧値（2026-05-08）のままだったため「残り -11日」と誤判定していた。
- **対策**: 有効期限を Meta Graph API の `debug_token`（`input_token`=`access_token`=システムユーザートークン）から**実測で取得**するよう変更。`expires_at`（unix秒）→ JST日付で残日数を算出。`is_valid=false` は失効扱い、`expires_at=0` は無期限として通知しない。
- **フォールバック**: API呼び出しに失敗した場合のみ従来の日付計算を使用（どちらで判定したかはログと Slack の「失効日（API実測/生成日から推定）」表示で分かる）。
- **運用**: `META_TOKEN_GENERATED_AT` の更新は必須ではなくなったが、フォールバック精度のため再発行時に更新しておくのが望ましい。
