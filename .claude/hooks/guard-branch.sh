...
#   （nagano の旧リポ名 *abil-ai-ops* 素通しで実証 — 2026-08-31 の昇格で削除）。
#   ★「旧リポ名」の 3 文字を落とさないこと。abil-os の tools/check-stale-refs.sh は
#     「経緯マーカー『旧』が付いていれば通す」設計で、外すと配布先が赤くなる
#     （2026-09-02 に正本を配った際に実際に落とし、別セッションが戻した）。
# ============================================================
